...
""" Specifications for the IPv6 literal domain name

The ipv6-literal.net domain name is a Microsoft creation:
https://devblogs.microsoft.com/oldnewthing/20100915-00/?p=12863

Below is a more concrete specification for the format, taking more use cases
into consideration, such as the non-numerical scope IDs on Linux and other
platforms.

An "IPv6 address literal domain name" represents an IPv6 address literal,
optionally including a scope ID, in a domain name format.

Specified in the usual ABNF (as used in RFCs, etc.):

    ipv6-address-literal-domain-name = ipv6-address-literal-label ".ipv6-literal.net"
    ipv6-address-literal-label = translated-address-bits [ "s" scope-id ]

"translated-address-bits" is generated by taking a colon-separated-hexadecimal
representation of the 128 address bits and replacing the colons ":" with
dashes "-".

(An implicit assumption here is that all characters appearing in scope-id
are valid characters in domain names.)

Examples:

::1 ==> --1.ipv6-literal.net
fe80::1ff:fe23:4567:890a%3 ==> fe80--1ff-fe23-4567-890as3.ipv6-literal.net
fe80::1ff:fe23:4567:890a%enp1s0 ==> fe80--1ff-fe23-4567-890asenp1s0.ipv6-literal.net
"""
...
class SOCKS5Acceptor:
    ...
    def _translate_domain_names(self, domain: str) -> Union[str, ipaddress.IPv6Address]:
        domain_lower = domain.lower()
        if not domain_lower.endswith(IPV6_LITERAL_HOST_NAME_SUFFIX):
            return domain
        translated_address_bits, sep, scope_id = domain_lower[:-len(IPV6_LITERAL_HOST_NAME_SUFFIX)].partition('s')
        address = ipaddress.IPv6Address(f"{translated_address_bits.replace('-', ':')}{'%' if sep else ''}{scope_id}")
        self._logger.info('Rewriting %s to %s', domain, address)
        return address
    ...
    async def accept(
        ...
        try:
            ...
            if addr_type is SOCKS5AddressType.IPV4_ADDRESS:
                ...
                try:
                    uhost = self._translate_domain_names(uhost)
                except ValueError:
